...
export const addSkillToUser = TryCatch(async (req: AuthenticatedRequest, res, next) => {
    ...
    if(!wasSkillAdded){
        return res.status(200).json({ message: "User already possess this skill" });
    }
...
export const deleteSkillFromUser = TryCatch(async (req: AuthenticatedRequest, res, next) => {
    ...
    const result = await sql`
        DELETE FROM user_skills WHERE user_id = ${user.user_id} AND skill_id = (
...
export const applyForJob = TryCatch(async (req:AuthenticatedRequest, res, next) => {
    ...
    try {
        [newApplication] = await sql`
            INSERT INTO applications (job_id, applicant_id, applicant_email, resume, subscribed) VALUES
...
export const getAllApplication = TryCatch(async (req:AuthenticatedRequest, res, next) => {
    const user = req.user;
