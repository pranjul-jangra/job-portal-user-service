...
// Adding additional type to Request 
...
export const isAuth = async (req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        const decodedPayload = jwt.verify(token as string, process.env.JWT_SECRET as string) as JwtPayload;
        ...
        if(users.length === 0){
            res.status(401).json({ message: "User associated with this token is no longer exists." });
            return;
        }
